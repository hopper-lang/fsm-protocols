module Data.Grammar.FSM.Reference where
...
protocol LibraryResource ( libOrg : OrgIdentity)  ( resource : ResourceUUID) where{
 ...
 -- states  = OnLoanLocal  | OnLoanInter | Reserved | Shelved | Returned
  ...
  states where {
      OnLocalLocal{heldBy = Identity ; dueBy = TAI.Time ; checkoutRequest : SignedBy RequestID heldBy ;
         checkedOutVia : DelagatedFrom Identity libOr ;
          CheckoutAccepted :   ; conditionAtCheckout = Maybe (ResourceCondition) }  ;
